...
namespace AnkleBreaker.Core.Utils
{
    /// <summary>
    ...
    /// </example>
    [Serializable]
    public class AB_SerializedDictionary<K, V> : AB_SerializedDictionary<K, V, K, V>
    {
        /// <summary>
    ...
    [Serializable]
    public abstract class AB_SerializedDictionary<K, V, SK, SV> : Dictionary<K, V>, ISerializationCallbackReceiver
    {
        ...
        public void OnBeforeSerialize()
        {
            ...
            {
                SK k = SerializeKey(kvp.Key);
                if (k == null)
                {
                    Debug.LogError($"{GetType().FullName} key is null.");
                    return;
                }

                SV v = SerializeValue(kvp.Value);
                if (v == null)
                {
                    Debug.LogError($"{GetType().FullName} value is null.");
                    return;
                }
